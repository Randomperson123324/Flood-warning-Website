  ...
  return (
    <div ref={rootRef} className="relative inline-block">
      <button
        ...
        type="button"
        onClick={() => setOpen((v) => !v)}
        className={cn("glass-panel-strong glass-interactive flex items-center gap-2 px-3 py-2 text-sm font-medium text-ink", className)}
        aria-haspopup="listbox"
        aria-expanded={open}
        aria-label={ariaLabel}
      >
        {triggerIcon}
        <span className={cn(hideLabelOnMobile && "hidden sm:inline", "truncate")}>{triggerLabel ?? selected?.label}</span>
