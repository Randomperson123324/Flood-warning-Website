...
-- Fixed INSERT policy to properly allow authenticated users to create their profile
...
-- Drop existing weather vote policies if they exist
DROP POLICY IF EXISTS "Anyone can view weather votes" ON weather_votes;
DROP POLICY IF EXISTS "Authenticated users can vote" ON weather_votes;
...
-- Drop existing affected areas policy if it exists
DROP POLICY IF EXISTS "Anyone can view affected areas" ON affected_areas;

-- Create policies for affected areas
CREATE POLICY "Anyone can view affected areas" ON affected_areas FOR SELECT USING (true);

-- Drop existing typing indicator policies if they exist
...
CREATE POLICY "Users can insert their own typing status" ON typing_indicators FOR INSERT WITH CHECK (auth.uid() = user_id);
